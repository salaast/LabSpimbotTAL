...
puzzle_part:
    ...
    sw $t0, 0($t8)      # rq puzzle w puzzlewrapper add
    ...
    while:
        ...
        #solve_queens(char **sol_board, int n, list_node_t *given_queens, int queens_to_place)

        # solve puzzle - jal solve_queens
        move $a0, $t1
        move $a1, $t9
        move $a2, $t2
        move $a3, $t3
...
request_puzzle_interrupt:
    ...
    # $t0 = unsigned_int board_size
    add $t1, $t0, 4     # char** board
    add $t2, $t0, 8     # list_node_t* given_queens
    add $t3, $t0, 12     # unsigned_int queens_to_place
    li $t5, 1
    la $t8, has_puzzle
    sw $t5, 0($t8)    # hopefully set has_puzzle
